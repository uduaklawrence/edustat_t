...
def create_invoice_record(user_id, total, data_dict):
    """
    Creates a new invoice with PENDING status and returns invoice_ref.
    The invoice is saved with status='pending' in invoice_data JSON field.
    """
    engine = create_connection()
    if engine is None:
        return None
    ...
    # Create initial invoice_data with pending status
    ...
    query = text("""
        INSERT INTO invoices (user_id, ref, total, data, invoice_data, created_at)
        VALUES (:user_id, :ref, :total, :data_json, :invoice_data, NOW())
    """)
...
def attach_paystack_ref_to_invoice(invoice_ref, paystack_ref):
    """Attach Paystack reference to existing invoice."""
    if not invoice_ref:
...
def mark_invoice_paid_by_paystack_ref(paystack_ref):
    ...
    query = text("""
        ...
        SET invoice_data =
            CASE
                WHEN invoice_data IS NULL THEN JSON_OBJECT('status', 'paid', 'paid_at', NOW())
                ELSE JSON_SET(invoice_data, '$.status', 'paid', '$.paid_at', NOW())
            END,
            updated_at = NOW()
...
# =========================================================
# 📁 SAVE USER REPORT (with report_name)
# =========================================================
def save_user_report(user_id, invoice_ref, report_group, report_name, filters, charts, pdf_path):
    """
    Saves a paid user report to user_reports table with user-defined report_name.
    expires_at auto-calculates as created_at + 30 days.
    """
    engine = create_connection()
    if engine is None:
        return None
    ...
    try:
        ...
        st.error(f"Failed to save user report: {e}")
        return None
...
def fetch_user_reports(user_id):
    """Fetch all active (non-expired) reports for a user."""
    query = """
        SELECT 
            report_id,
            invoice_ref,
...
# =========================================================
def fetch_invoice_by_ref(invoice_ref):
    """Fetch invoice details by reference number."""
    query = """
        SELECT 
            invoice_id,
            user_id,
        ...
        WHERE ref = :ref
    """
    return fetch_data(query, {"ref": invoice_ref})


# =========================================================
# 📁 Fetch User Invoices
# =========================================================
def fetch_user_invoices(user_id):
